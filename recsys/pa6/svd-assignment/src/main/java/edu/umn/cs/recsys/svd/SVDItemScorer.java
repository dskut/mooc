...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector scores) {
        ...
        for (VectorEntry e: scores.fast(VectorEntry.State.EITHER)) {
            long item = e.getKey();
            // TODONE Set the scores
            
            RealMatrix itemVector = model.getItemVector(item);
            double base = baselineScorer.score(user, item);            
            double prediction = userVector.multiply(weights).multiply(itemVector.transpose()).getEntry(0, 0);
            //logger.info("scoring {} => {}, base = {}, prediction = {}", user, item, base, prediction);
            scores.set(item, base + prediction);
        }
    }
